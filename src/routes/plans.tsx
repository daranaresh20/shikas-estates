...
export const Route = createFileRoute("/plans")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "House Plans — Home" },
    ...
    <Layout>
      <section className="px-4 sm:px-6 lg:px-8 pt-12 sm:pt-16 pb-8">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="font-display text-4xl sm:text-5xl mt-2 leading-[1.05]">Six typologies. Endless customisation.</h1>
          <p className="mt-3 text-sm sm:text-base text-[var(--muted-sage)] max-w-2xl leading-relaxed">Each plan is the start of a conversation. Pick a shape we love, then we shape it around your life.</p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto flex flex-wrap gap-2">
          {CATS.map((c) => (
            <button
              key={c}
              onClick={() => setCat(c)}
              className={`px-4 py-2 rounded-full text-xs sm:text-sm border transition min-h-[38px] ${
                cat === c
                  ? "bg-[var(--gold)] text-[var(--forest)] border-[var(--gold)] shadow-[0_8px_24px_-10px_rgba(212,165,116,0.6)] font-mono uppercase tracking-wider"
      ...
      <section className="px-4 sm:px-6 lg:px-8 mt-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((p) => (
            <article key={p.id} className="luxe-card rounded-xl overflow-hidden group flex flex-col justify-between">
              <div>
                <div className="relative aspect-[4/3] sm:aspect-[5/4] overflow-hidden bg-slate-100">
                  <img src={p.image} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                  <span className="absolute top-3 left-3 text-[10px] font-semibold uppercase tracking-widest px-2 py-1 rounded bg-[var(--forest)]/80 text-[var(--gold)] border border-[var(--gold)]/40 font-mono">{p.category}</span>
                </div>
                <div className="p-5">
                  <h3 className="font-display text-xl leading-tight">{p.name}</h3>
                  <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-[var(--cream-2)]/85">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-4">
                    {p.features.map((f) => (
                      <span key={f} className="text-[10px] uppercase tracking-wider text-[var(--cream-2)]/85 border border-[var(--gold)]/25 px-2 py-0.5 rounded font-mono">{f}</span>
                    ))}
                  </div>
                </div>
              </div>
              <div className="p-5 pt-0 border-t border-[var(--gold)]/5 mt-2 flex items-center justify-between gap-3">
                ...
                <Button variant="gold" size="sm" className="h-9 px-4" onClick={() => { setActiveImgIdx(0); setSelected(p); }}>View Plan</Button>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-[var(--forest)]/85 backdrop-blur-sm grid place-items-center p-4 overflow-y-auto" onClick={() => setSelected(null)}>
          <div className="max-w-4xl w-full bg-[var(--forest-2)] border border-[var(--gold)]/30 rounded-xl overflow-hidden my-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 sm:p-6 md:p-8 grid md:grid-cols-2 gap-8">
              <div>
                <p className="text-xs uppercase tracking-widest text-[var(--gold)] font-mono">{selected.category}</p>
                <h3 className="font-display text-3xl mt-1 leading-none">{selected.name}</h3>
                ...
                <ul className="mt-2 space-y-1.5 text-sm text-[var(--cream-2)]/85">
                  {selected.features.map((f) => <li key={f} className="font-mono text-xs">• {f}</li>)}
                </ul>
              </div>
